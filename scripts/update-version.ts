...
import { errorAndExit, getWorkspacePackages } from '@lightjs/build-utils'
...
async function main() {
  ...
  const elementPlus = pkgs['element-plus'] || pkgs['@lightjs/nightly']
  const eslintConfig = pkgs['@lightjs/eslint-config']
  const metadata = pkgs['@lightjs/metadata']
